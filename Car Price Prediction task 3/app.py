...
features = ['wheelbase', 'carlength', 'carwidth', 'curbweight', 'enginesize', 'boreratio', 'horsepower',
            ...
            'carbody_sedan', 'carbody_wagon', 'drivewheel_4wd', 'drivewheel_fwd', 'drivewheel_rwd',
            'enginetype_dohc', 'enginetype_dohcv', 'enginetype_l', 'enginetype_ohc', 'enginetype_ohcf',
            'enginetype_ohcv', 'enginetype_rotor', 'cylindernumber_eight', 'cylindernumber_five',
            'cylindernumber_four', 'cylindernumber_six', 'cylindernumber_three', 'cylindernumber_twelve',
            'cylindernumber_two', 'fuelsystem_1bbl', 'fuelsystem_2bbl', 'fuelsystem_4bbl', 'fuelsystem_idi',
            'fuelsystem_mfi', 'fuelsystem_mpfi', 'fuelsystem_spdi', 'fuelsystem_spfi', 'CarsRange_Budget',
...
primary_features = numcols + nominal

models = ["LinearRegression", "DecisionTree",
          "RandomForest", "LGBM", "XGBoost", "CatBoost"]

df = pd.read_csv('helperdata.csv')
...
@app.route('/sample')
def sample():
    dd=df[primary_features].sample(1).to_dict('records')[0]
    return render_template("index.html",
                           param=dd)

@app.route('/')
def index():
    param = defaultdict(None)
    return render_template("index.html",
                           param=param)
